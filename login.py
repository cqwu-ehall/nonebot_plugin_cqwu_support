...
async def _(__: PrivateMessageEvent, state: T_State, username: str = ArgPlainText('学号')):
    if username == '退出':
...
async def _(event: PrivateMessageEvent, state: T_State, password: str = ArgPlainText('密码')):
    if password == '退出':
        await login_cqwu.finish("🚪已成功退出")
    ...
    text = f"欢迎你，{client.me.institute} {client.me.now_class} 的 {client.me.name}，" \
           f"你可以使用 /cqwu 查询相关命令。"
    cqwu_data.add_user(int(event.user_id), username=state['username'], password=password, client=client)
